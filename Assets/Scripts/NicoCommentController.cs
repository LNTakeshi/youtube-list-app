...
public class NicoCommentController : MonoBehaviour
{
    ...
    public void setComment(int index,String XMLString){
        commentDatas[index] = new List<CommentData>();
        try{
            XDocument xml = XDocument.Parse(XMLString);

            //テーブルを読み込む
            XElement table = xml.Element("packet");
            ...
            {   
                commentDatas[index].Add(new CommentData(row?.Value, Int32.Parse(row.Attribute("vpos")?.Value ?? "0"), row.Attribute("mail")?.Value, Int32.Parse(row.Attribute("score")?.Value ?? "0")));
            }
            //ソート
